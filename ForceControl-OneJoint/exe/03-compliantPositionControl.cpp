...
void run() {
	...
	double fAvg = 0.0;
	int	fHistSize = 50;
	double fHistory[fHistSize]; for(int i = 0; i < fHistSize; i++) fHistory[i] = 0.0;
	int fIter = 0;
	...
	while(!somatic_sig_received) {
		...
		fAvg += (- fHistory[fIter] + f)/fHistSize;
		fHistory[fIter] = f;	
		fIter = (fIter + 1) % fHistSize;
		//if(c++%10 == 0) { for(int i = 0; i < 6; i++) cout << f(i) << "\t"; cout << endl; }

		// If position is not zero then control position compliantly, when position is zero control force
		double error = qref - q; 
		...
		//cout << "f = " << f << ",\t-(1/Km)*(1/Kx)*f*r = " << -(1/Km)*(1/Kx)*f*r;
		//cout << ",\t(1/Km)*f*r = " << (1/Km)*f*r << endl;
		//cout << "u (" << u << ") = XX*w (" << XX*w << ") + g*MY*cos(q) (" << g*MY*cos(q);
		//cout << ") + g*MZ*sin(q) (" << g*MZ*sin(q) << ") + Friction (";
		//cout << ((abs(dq) > 0.5 & abs(qref-q) > 0.1 ) ? ( dq > 0.0 ? (Fc1 + Fv1*dq) : (Fc2 + Fv2*dq)  ) : 0);
