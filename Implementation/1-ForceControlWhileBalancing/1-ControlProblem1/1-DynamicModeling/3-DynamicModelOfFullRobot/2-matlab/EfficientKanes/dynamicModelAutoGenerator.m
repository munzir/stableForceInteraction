...
dq = dqVec(f);
ddq = ddqVec(f);
fout = fopen('dynamicModelAuto.m','w');
...
for i=1:length(dq)
    ...
    for j=1:length(keys)
        b = keys{j};
        LHSTmp = LHSTmp + f(b).maG'*diff(f(b).ve, dq(i)) + f(b).dHnew'*diff(f(b).we, dq(i));
    end
...
for i=1:length(dq)
    ...
    for j=1:length(keys)
        b = keys{j};
        GTmp = GTmp + mass(f, b)*f(b).g'*diff(f(b).ve, dq(i)) + f(b).mSxg'*diff(f(b).we, dq(i));
    end
...
for i=1:length(keys)
    this = keys{i};
    p = f(this).wi;
    fprintf(fout, 'wi_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).we;
    fprintf(fout, 'we_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).vi;
    fprintf(fout, 'vi_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).ve;
    fprintf(fout, 've_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).alpha;
    fprintf(fout, 'alpha_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).acc;
    fprintf(fout, 'acc_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).g;
    fprintf(fout, 'g_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
end

for i=1:5; fprintf(fout, '%s\n','% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'); end

Z.print(fout);
for i=1:length(keys)
    this = keys{i};
    p = f(this).mSxg;
    fprintf(fout, 'mSxg_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).maG;
    fprintf(fout, 'maG_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
    p = f(this).dHnew;
    fprintf(fout, 'dHnew_%s = [%s;%s;%s];\n', this, char(p(1)), char(p(2)), char(p(3))); 
end
...
zL.print(fout);
for i=1:length(dq)
    fprintf(fout, 'LHS(%d) = %s;\n', i, char(collect(LHS(i), ...
        [ddx, ddpsi, ddq_imu, ddq_w, ddq_torso,...
